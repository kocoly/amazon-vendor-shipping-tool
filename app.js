...
const DEFAULT_BOX_SPECS = [
  ...
  { name: "套装 四边形27x20英寸柔光箱 单温灯 SL21 UB 美规", asin: "B09XDMTYJL", pcsPerBox: 4, singleVol: 5.07, singleWeight: 38.50 },
  ...
  { name: "套装 L型手机夹 I UB+蓝牙遥控器", asin: "B07SSZRXD5", pcsPerBox: 150, singleVol: 3.99, singleWeight: 22.00 },
  { name: "套装 L型磁吸手机夹I+蓝牙遥控 UB", asin: "B0CY4KW1ZM", pcsPerBox: 150, singleVol: 2.06, singleWeight: 220.00 },
  ...
  { name: "套装 22*14*4.5cm空气炸锅纸250PCS UB", asin: "B0DSVMB6FH", pcsPerBox: 28, singleVol: 3.00, singleWeight: 47.30 },
  { name: "套装 虎口悬臂支架+10寸软管款 UB", asin: "B0CYT4KB4N", pcsPerBox: 10, singleVol: 4.63, singleWeight: 34.54 },
  { name: "套装 三脚架A 黑I UB", asin: "B088GNC6TT", pcsPerBox: 60, singleVol: 2.69, singleWeight: 34.10 },
  { name: "套装 62寸II +12寸触控款 UB", asin: "B08L5VKNWR", pcsPerBox: 5, singleVol: 2.64, singleWeight: 15.40 },
  { name: "套装 62寸II +12寸RGB五键线控款 UB", asin: "B0DSHGMQ6F", pcsPerBox: 5, singleVol: 2.45, singleWeight: 13.73 },
  { name: "套装 自拍脚架62寸 II UB", asin: "B07NWC3L95", pcsPerBox: 30, singleVol: 3.15, singleWeight: 46.20 },
  ...
  { name: "套装 反折脚架67英寸+38cm软管 黑色 UB", asin: "B0C6LNFHTP", pcsPerBox: 18, singleVol: 2.00, singleWeight: 43.78 },
  ...
  { name: "套装 C72磁吸一体式自拍杆 黑色 UB", asin: "B0CY2NH57B", pcsPerBox: 30, singleVol: 2.40, singleWeight: 34.21 },
  { name: "XS64 方形磁吸一体式自拍杆 黑色 UB", asin: "B0D2R8X92Z", pcsPerBox: 34, singleVol: 1.58, singleWeight: 31.90 },
  { name: "套装 自拍脚架72英寸+38cm软管 黑色 UB", asin: "B0CSFW56YF", pcsPerBox: 20, singleVol: 2.30, singleWeight: 38.83 },
  { name: "套装 自研悬臂+16W 10寸全屏灯 UB", asin: "B0DC63991M", pcsPerBox: 10, singleVol: 4.33, singleWeight: 28.56 },
  { name: "XS64 方形磁吸一体式自拍杆 白色 UB", asin: "B0DNSPXC1Q", pcsPerBox: 34, singleVol: 1.58, singleWeight: 31.90 },
...
function searchBoxSpecs() {
  const q = document.getElementById("searchAsinInput").value.trim().toUpperCase();
  renderSpecTable(boxSpecDB.filter(x => x.asin.toUpperCase().includes(q) || (x.name && x.name.includes(q))));
}
...
function editSpec(asin) {
  const item = boxSpecDB.find(x => x.asin === asin);
  if (!item) return;
  editingAsin = asin;
...
function saveSpec() {
  ...
  const pcs = parseInt(document.getElementById("modalPcs").value) || 0;
  const vol = parseFloat(document.getElementById("modalVol").value) || 0;
  const wt = parseFloat(document.getElementById("modalWeight").value) || 0;

  if (!asin) return alert("请输入 ASIN！");

  if (editingAsin) {
    const item = boxSpecDB.find(x => x.asin === editingAsin);
    if (item) { item.name = name; item.pcsPerBox = pcs; item.singleVol = vol; item.singleWeight = wt; }
  } else {
    if (boxSpecDB.some(x => x.asin === asin)) return alert("该 ASIN 已存在！");
    boxSpecDB.push({ name, asin, pcsPerBox: pcs, singleVol: vol, singleWeight: wt });
  }
  saveDB();
  closeSpecModal();
}
...
function deleteSpec(asin) {
  if (confirm(`确定删除 ASIN ${asin} 的箱规吗？`)) {
    boxSpecDB = boxSpecDB.filter(x => x.asin !== asin);
    saveDB();
  }
}

// ---------------- 2. 文本正则解析 (ARN & Shipment编号) ----------------
function parseArnText(rawText) {
  const map = new Map();
  ...
  while ((match = regex.exec(rawText)) !== null) {
    map.set(match[2].trim(), {
      shipmentNo: match[1].trim(),
      arn: match[3].trim()
    });
  }
  return map;
...
async function processShippingData() {
  ...
  const arnMap = parseArnText(document.getElementById("arnText").value);
  ...
  rawRows.forEach(row => {
    ...
    const pcs = parseFloat(row["PCS"] || row["Requested quantity"] || 0);
    const confirmed = parseFloat(row["确认数量"] || pcs);
    const location = String(row["Ship-to location"] || "").trim();
    const warehouse = String(row["仓库"] || "").trim();
    const expectDate = row["Expected date"] || row["Expected Date"] || "";
    ...
    const cartons = spec.pcsPerBox > 0 ? Math.ceil(confirmed / spec.pcsPerBox) : 0;
    const vol = cartons * spec.singleVol;
    const wt = cartons * spec.singleWeight;

    // 加拿大托盘计算规则
    let pallets = 0;
    if (cartons > 10) {
      const pVol = Math.ceil(vol / 68.96);
      const pWt = Math.ceil(wt / 1400);
      pallets = Math.max(pVol, pWt);
    }
    ...
    initData.push({
      ...
      "Expected date": expectDate
    });
  });

  // --- B. 生成：表一 ---
  const table1 = [];
  initData.forEach((row, idx) => {
    const t1Row = {
      "PO Number": row["PO"],
      "PO Destination": row["Ship-to location"],
      "ASIN/ESKU": row["ASIN"],
      "Confirmed": row["确认数量"]
    };
    initData.forEach((_, sIdx) => {
      t1Row[`Shipment ${sIdx + 1}`] = (sIdx === idx) ? row["确认数量"] : 0;
    });
    table1.push(t1Row);
  });

  // --- C. 生成：表二 ---
  const rowDestination = { "Input": "Desination:" };
  const rowShipAsin = { "Input": "Ship ASIN" };
  const rowTotalUnits = { "Input": "Total Units:" };
  const rowPickDate = { "Input": "Requested pick up date" };
  const rowPickLoc = { "Input": "Pick up location" };
  const rowStacked = { "Input": "Stacked pallets" };
  const rowUnstacked = { "Input": "Unstacked pallets" };
  const rowCartons = { "Input": "Cartons" };
  const rowWeight = { "Input": "Total weight (lbs.)" };
  const rowVolume = { "Input": "Total volume (cuFt.)" };
  const rowRefNum = { "Input": "Shipment reference number (optional)" };

  initData.forEach((row, i) => {
    const shipKey = `Shipment ${i + 1}`;
    rowDestination[shipKey] = row["Ship-to location"];
    rowShipAsin[shipKey] = row["ASIN"];
    rowTotalUnits[shipKey] = row["确认数量"];
    rowPickDate[shipKey] = row["Expected date"];
    rowPickLoc[shipKey] = row["仓库"];
    rowStacked[shipKey] = "";
    rowUnstacked[shipKey] = row["托盘"];
    rowCartons[shipKey] = row["箱数"];
    rowWeight[shipKey] = row["重量"].toFixed(2);
    rowVolume[shipKey] = row["体积"].toFixed(2);
    rowRefNum[shipKey] = "";
  });

  const table2Rows = [
    rowDestination, rowShipAsin, rowTotalUnits, rowPickDate,
    rowPickLoc, rowStacked, rowUnstacked, rowCartons,
    rowWeight, rowVolume, rowRefNum
  ];

  // --- D. 生成：总表 ---
  const summaryData = initData.map((row, idx) => {
    const po = row["PO"];
    const asin = row["ASIN"];
    const spec = specMap.get(asin) || { singleVol: 0, singleWeight: 0 };
    const matchArn = arnMap.get(po) || { shipmentNo: idx + 1, arn: "" };

    return {
      "提货日期": row["Expected date"],
      "PO-ASIN": po + asin,
      "window start": row["window start"],
      "window end": row["window end"],
      "shipment编号": matchArn.shipmentNo,
      "ARN": matchArn.arn,
      "PO": po,
      "ASIN": asin,
      "PCS": row["PCS"],
  ...
  generatedResult = { initData, table1, table2Rows, summaryData };

  renderHtmlTable(summaryData, "tableSummary");
  renderHtmlTable(table1, "table1");
  renderHtmlTable(table2Rows, "table2");
  renderHtmlTable(initData, "tableInit");

  document.getElementById("resultCard").style.display = "block";
  document.getElementById("downloadAllBtn").style.display = "inline-block";
}

function renderHtmlTable(dataRows, elementId) {
